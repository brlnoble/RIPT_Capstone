...
class YoLov7TRT(object):
    ...
    def infer(self, image):
        ...
        return image_raw, end - start, [result_classid.astype(int),result_boxes] #~~~~~~~~~ADDED RETURN RESULT BOXES~~~~~~~~~
    ...
    def cool_bbox_iou(self, box1, box2, x1y1x2y2=True):
        """
        description: compute the IoU of two bounding boxes
        param:
            box1: A box coordinate (can be (x1, y1, x2, y2) or (x, y, w, h))
        ...
        return 1 if iou > 0.03 else 0 #Threshold of 0.03 for now
...
class inferThread(threading.Thread):
    ...
    def run(self):
        ...
        #Window to show the inference
        window = sgInterface.GetDisplayWindow()
        ...
        while(True):
            ...
            event, values = window.read(timeout=1) # ~~~~~ ADDED THIS ~~~~~~~
            ...
            # ~~~~~ ADDED THIS ~~~~~
            infer_img = cv2.imencode('.png',result)[1].tobytes()
            ...
            if cv2.waitKey(1) & 0xFF == ord('q') or event == "exit_btn":
